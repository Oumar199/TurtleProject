...
# une fonction (ou procédure) qui comportera les manipulations permettant de tracer la figure

# Mais d'abord créons quelques fonctions trés utiles au programme
def set_move(x, y: float = None):
    ...
    Args:
        x(float ou tuple ou list): si c'est une liste ou un tuple goto le prendra seul comme para-
...
def carre(cote: float):
    """Cette fonction nous permet de tracer un carré
    Method:
        On utilise la boucle for pour répéter les fonctions forward et left(90 degrés). Ce qui nous
        permet de tracer les quatre côtés du carré d'égales longueurs
    Args:
        cote(float): La longueur de chaque côté du carré. Cette longueur ne doit être inferieure ou
        égale a 0
    Returns:
        None
    """
...
def rectangle(
    ...
    Args:
        ...
        largeur(float): la largeur du rectangle qui ne doit pas non plus être inferieure ou égale à 0
    Returns:
        None
    """
    if (longueur > 0  and
        ...
        raise ValueError("La longueur et la largeur ne doivent pas être inférieures ou égales a 0")
...
def ellipse(rayon: float):
    ...
    Method:
        ...
        -mètre et un quart de cercle de rayon dont la longueur est la partie entière de la longueur du rayon donné en paramètre 
        divisée par deux. Ces tracées de quart de cercle se feront à l'aide de la fonction circle de turtle
    Args:
        rayon(float): la longueur du rayon utilisé sur deux quarts du cercle. La longueur du rayon doit être
        supérieure à 2 pour éviter que la partie entière de la longueur du rayon divisée par deux soit égale 0
    Returns:
        None
    """
    if rayon > 2:
...
def triangle(
    ...
    Method:
        On récupère l'angle droit et l'angle à l'opposé de la base du triangle à l'aide de la fonction d'Al-Kachi. On vérifie ensuite si le
        ...
        On trace le deuxième coté avec forward(cote_2), puis tourner a 180 - angle2 (angle à l'opposé de la base) et enfin
        On trace le troisième côté avec forward(cote_3).
    Args:
        cote_1(float): la longueur de la base du triangle qui doit être supérieure à 0 et inférieure à la somme des deux autres côtés
        du triangle
        cote_2(float): la longueur du côté droit du triangle qui doit être supérieure à 0 et inferieure à la somme des deux autres côtés
        cote_3(float): la longueur du côté gauche du triangle qui doit être supérieure à 0 et inferieure à la somme des deux autres
        côtés
        up(bool): la valeur booléenne qui doit indiquer si le sommet du triangle pointe vers le côté supérieur à la base ou vers 
        le côté inférieur à la base du trapèze. Si elle n'est pas indiquée alors elle prendra par défaut la valeur True
    Returns: 
        None
    """
    if (cote_1 > 0 and
...
def losange(cote: float, diag = rdt(10, 50)):
    ...
    Method:
        ...
        Pour concevoir les deux triangles on va tourner à 180 - angle degrés (angle droit de chaque triangle), avancer avec
        forward(cote), tourner a 180 - angle2 degrés (angle à l'opposé du diagonale du losange) et avancer avec forward(cote)
        Les différences résident aux points suivants pour chaque triangle :
        - triangle 1: on lève le crayon avec penpup(), on avance avec forward(diag) et on remet le crayon en place (pendown())
        - triangle 2: on tourne à "angle" degrés à droite
    Args:
...
def trapeze(cote_1: float, cote_2: float, cote_3 = rdt(20, 40), angle: float = 90, up: bool = True):
    ...
    Method:
        ...
        avec forward(longueur côté droit du trapèze), on tourne à angle (angle droit du trapèze) degrés pour ensuite avancer avec                   
        forward(longueur côté parallèle à la base) pour enfin avancer le crayon jusqu'à la position initiale avec goto(position ini-
        -tiale)
    Args:
        cote_1(float): la longueur de la base du trapèze qui doit être supérieure à la longueur du côté qui lui est opposé et
        supérieure à 0
        cote_2(float): la longueur du côté du trapèze qui doit être supérieure à 0
        cote_3(float): la longueur du côté opposé à la base qui doit être supérieure à 0 et à la longueur de la base. Si cette longueur
        ...
        up(bool): la valeur booléenne qui doit indiquer si le côté opposé de la base du trapèze sera conçu vers le côté supérieur à la             
        base du trapèze ou vers le côté inférieur à la base du trapèze. Si elle n'est pas indiquée alors elle prendra par défaut la valeur True 
    Returns:
        None
    """
