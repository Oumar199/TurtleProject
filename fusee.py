...
def fusee():
    ...
    positions = {}
    #Tracons le premier triangle et stockons sa position avant pour pouvoir l'utiliser apres
    #Les longueurs des figures seront stockees dans des variables comme suivants
    ltri1 = 66
    ...
    #On reprend la position de depart pour tracer un carre rouge au milieu du triangle et stockage de sa position
    set_move(positions["tri1"][0] + 18, positions["tri1"][1])
    lcar1 = (ltri1 - 6) / 2
    fillcolor("red"); begin_fill(); carre(lcar1); end_fill()
    positions["carre1"] = pos()
    
    #Tracons un carre juste en dessous apres avoir defini une position suivant la position
    ...
    # Pour tracer on va utiliser la fonction setheading(angle)
    setheading(0)
    ...
    #On trace un rectangle a cote des carres de bleu foncé (on souhaite que l'on ai l'impression
    # de voir des ombres la fusee
    set_move(positions["rec1"][0], positions["rec1"][1] - Lrec1)
    setheading(0)
    fillcolor("darkblue"); begin_fill(); rectangle(lrec1, lcote2_rc1); end_fill() 
    positions["rec2"] = pos()
    
    #La fonction set_move() sera utilisé a plusieurs et semble etre l'une des fonctions les plus utiles
    #de ce projet
